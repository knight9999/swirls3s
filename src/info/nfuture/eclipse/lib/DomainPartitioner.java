...
public class DomainPartitioner implements IDocumentPartitioner,
	...
	protected final DomainScanner fScanner;
	/** The legal content types of this partitioner */
	protected final String[] fLegalContentTypes;
	/** The partitioner's document */
	protected DomainDocument fDocument;
	 ...
	 */
	public DomainPartitioner(DomainScanner scanner, String[] legalContentTypes) {
		fScanner= scanner;
	...
	public void connect(IDocument document, boolean delayInitialization) {
		...
		fDocument= (DomainDocument) document;
		fDocument.addPositionCategory(fPositionCategory);
		fDocument.setDomainPositionCategory(fPositionCategory);
		...
		try {
			...
			while (!token.isEOF()) {
				...
				if (isSupportedContentType(contentType)) {
					DomainPosition p= new DomainPosition(fScanner.getTokenOffset(), fScanner.getTokenLength(),
							new DomainTag( token , fScanner.getKey() ),
							fScanner.getNextType());
					fDocument.addDomainPosition(p);
					prev = p;
				}
	...
	public void disconnect() {
		...
		try {
			...
			fDocument.setDomainPositionCategory(null);
		} catch (BadPositionCategoryException x) {
	...
	public IRegion documentChanged2(DocumentEvent e) {
		...
		try {
			...
			String rkey = null;
			if (first > 0)	{
				DomainPosition partition= (DomainPosition) category[first - 1];
				if (partition.includes(reparseStart)) {
					// リパース開始位置（行頭）が、そのパーティション内に含まれる場合
					partitionStart= partition.getOffset();
					contentType= partition.getType();
					token = partition.getTag().getToken();
					if (e.getOffset() == partition.getOffset() + partition.getLength())
						reparseStart= partitionStart; // partitionStartから読み込む (それ以外の場合は、reparseStart開始位置から読むので経済的）
					-- first;
				} else if (reparseStart == e.getOffset() && reparseStart == partition.getOffset() + partition.getLength()) {
					// 前の行末でそのパーティションが終わっている場合 ... タイプ２
					partitionStart= partition.getOffset();
					contentType= partition.getType();
					token = partition.getTag().getToken();
					reparseStart= partitionStart;  // rpartitionStartから読み込む
					rkey = partition.getTag().getKey();
					-- first;
				} else {
					// 前のパーティションはすでに終わっていて、デフォルトコンテントタイプの場合
			...
			for (int i= first; i < category.length; i++) {
				...
				/* fPositionUpdater.update(e)で、長さ0にされてしまった
				 * DomainPositionは、nextTypeの設定をなしにする
				 */
				DomainPosition dp = (DomainPosition) p;
				if (dp.length == 0 && e.fOffset == p.offset) {
					dp.nextType = null;
				}
			}
			clearPositionCache();
			...
			if (rkey != null) { // タイプ２のとき専用の処理
				fScanner.setKey( rkey );
			}
			
			int behindLastScannedPosition= reparseStart;
			token= fScanner.nextToken();
//			DomainPosition [] arr2 = fDocument.getDomainPositions(); // debug
//			System.out.println(arr2.length); // debug
			...
			while (!token.isEOF()) {
				...
				while (first < category.length) {
					DomainPosition p= (DomainPosition) category[first];
					if (lastScannedPosition >= p.offset + p.length ||
							(p.overlapsWith(start, length) &&
							 	(!fDocument.containsPosition(fPositionCategory, start, length) ||
							 	 !contentType.equals(p.getType()) ||
							 	 (p.getTag().getKey() != null) ))) {

						rememberRegion(p.offset, p.length);
						removeNum ++;
						fDocument.removeDomainPosition(p);
						++ first;
				...
				if (fDocument.containsPosition(fPositionCategory, start, length)) {
					...
					try {
						DomainPosition p= new DomainPosition(start,length,
								new DomainTag( token , fScanner.getKey() ),
								fScanner.getNextType() );
						DomainPosition [] arr = fDocument.getDomainPositions();
						for (int i=0;i<arr.length;i++) {
							if (arr[i].getOffset() >= p.getOffset()) {
								for (int j=i;j<arr.length;j++) {
									DomainPosition ap = arr[j];
									if (p.getOffset()+p.getLength()>ap.getOffset()) {
										fDocument.removeDomainPosition(ap);
										rememberRegion(ap.offset, ap.length);
									}
								}
								clearPositionCache();
								break;
						...
						arr = fDocument.getDomainPositions();

						
						
						fDocument.addDomainPosition(p);
						DomainPosition prev = p.getPrev();
						if (prev != null && prev.getOffset()+prev.getLength()>p.getOffset()) {
							prev.setLength( p.getOffset() - prev.getOffset());
						}
			...
			DomainPosition p;
			while (first < category.length) {
				p= (DomainPosition) category[first++];
				fDocument.removeDomainPosition(p);
				rememberRegion(p.offset, p.length);
			}
	 ...
	 */
	protected DomainPosition findClosestPosition(int offset) {
		...
		try {
			...
			if (index < category.length) {
				if (offset == category[index].offset)
					return (DomainPosition) category[index];
			}
	...
	public String getContentType(int offset) {
		...
		DomainPosition p= findClosestPosition(offset);
